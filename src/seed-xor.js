...
import { getDeterministicEntropyFromMnemonic, getRandomEntropy, bitwiseXorHexString, mnemonicToEntropyLength, } from './utils.js';
// https://github.com/Coldcard/firmware/blob/master/shared/xor_seed.py
export const split = async (mnemonic, numberOfShares = 2, useRandom = false) => {
    ...
    for (let i = 0; i < numberOfShares - 1; i++) {
        shares.push(useRandom
            ...
            : await getDeterministicEntropyFromMnemonic('Batshitoshi ', mnemonic, i, numberOfShares, entropyLength));
    }
    shares.push(bitwiseXorHexString([bip39.mnemonicToEntropy(mnemonic), ...shares]));
...
export const combine = async (shares) => {
